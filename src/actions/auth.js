...
const logout = () => (dispatch, getState) => { //here we used () instaed of login data because there is no data
  dispatch({
    type: LOGOUT
  });
...
const token = getState().auth.login.token; //the demo of kwitter reducers/actions at 42 min shows how we found the token inside the reducers/auth file. 
  ...
  return fetch(url + "/logout", {
    ...
    .then(result => {//here it says the fetch was successful and here is your result which contain the token which a string of contents
      // go back to the homepage
      //dispatch(push("/"));
      return dispatch({//here it says please dispatch the type of dispatch (LOGOUT_SUCCESS)and payload of dispatch (the result content)
        type: LOGOUT_SUCCESS,
        payload: result
      });
    })
